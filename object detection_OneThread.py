...
config.enable_stream(rs.stream.depth, 1280, 720, rs.format.z16, 30) # 추가
...
while True:
    ...
    output_dict = (boxes, scores, classes, num)
    ...
    # Crop depth data:
    depth = boxes.astype(float)

    # Get data scale from the device and convert to meters
    depth_scale = profile.get_device().first_depth_sensor().get_depth_scale()
    depth = depth * depth_scale
    dist, _, _, _ = cv2.mean(depth)
    print("Detected a {0} {1:.3} meters away.".format(num_detections, dist))
